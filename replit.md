...
-   **Warri Crusade 2026 Admin Tile:** `WarriCrusadeStatsTile` (`artifacts/jctm-platform/src/components/admin/WarriCrusadeStatsTile.tsx`) is rendered at the top of the **Events** section in the Admin dashboard. It auto-refreshes every 30 s from `GET /api/admin/warri-crusade/stats` (livestream/sermon/gallery role required) and shows: campaign live/ended badge with countdown to end, four KPI cards (active subscribers, broadcasts sent, average delivery rate, click-through count + CTR%), the next scheduled hourly broadcast (relative + WAT clock time), a 24-hour stacked-bar + line chart (sent vs failed bars, clicks line), and a list of the 20 most-recent `broadcast_events` rows for the campaign. Includes a **"Send broadcast now"** button (visible only while the campaign is active) that opens a confirmation modal showing the notification preview + active-subscriber count, then calls `POST /api/admin/warri-crusade/broadcast-now` (livestream role) which delegates to the exported `broadcastWarriCrusadeManual()` helper in `cron.ts`. The helper enforces a **5-minute cooldown** by querying `broadcast_events` for any prior row whose `message` carries the synthetic `[manual:…]` tag inside the cooldown window — duplicate clicks return HTTP 429 with the cooldown reason; successful sends fan out the same payload as the hourly cron and toast the result (delivered / failed / deactivated counts) on the admin dashboard. Hourly auto-broadcasts continue independently and use a different (`[hour:…]`) dedup key, so a manual send never blocks the next scheduled hour. Click-through tracking flows from the SW `notificationclick` handler — it fires a fire-and-forget POST to `/api/push/track-click` (with `keepalive`) before navigating, recording each click in the new `notification_clicks` table (`broadcast_type`, `target_url`, `visitor_id`, `clicked_at`) so CTR is computed as `clicks / total_sent` per campaign. The endpoint is unauthenticated by design (anyone tapping a notification triggers it) but values are length-clamped before insert. Index `notification_clicks_type_idx` keeps the per-campaign aggregations fast. Service worker bumped to `CACHE_VERSION = "jctm-v5"` so existing PWA installs pick up the new click handler on next activation.
-   **Warri Crusade 2026 Hourly Multi-Channel Broadcast:** `checkAndBroadcastWarriCrusadeHourly` in `artifacts/api-server/src/lib/cron.ts` is invoked from the existing per-minute scheduler tick. It fires **once per UTC hour bucket** (between minutes 00–04 of each hour) from now until the campaign end (`2026-05-01T21:00:00+01:00`), then disables itself permanently. Each fire dispatches (a) a web-push notification (`title: "🔥 Warri Crusade 2026 Update"`, `body: "Join the powerful move of God today"`, `url: /crusade`, `data.broadcastType: "event_reminder"` — re-using the existing yellow "Coming Soon" SW relay palette so connected clients see an in-app sonner toast) via `dispatchPushNotification`, and (b) inserts a `broadcast_events` row (`type: "warri_crusade_promo"`, `message` includes the hour bucket key) which serves both as the **idempotency lock** (exact-match check before insert prevents duplicate sends within the same hour, even across process restarts) and as the source for the SSE missed-broadcast re-engagement card. Email is intentionally NOT included in this hourly fan-out to avoid spamming the devotion subscriber list — the existing daily devotion email pipeline remains the email channel of record. All dispatches are logged to `push_dispatch_log` via the standard pipeline.
