...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  const existing = await db.select().from(memberAuthTable).where(eq(memberAuthTable.email, email.toLowerCase())).limit(1);
  ...
  const [member] = await db.insert(memberAuthTable).values({
...
router.post("/auth/login", async (req, res): Promise<void> => {
  ...
  const [member] = await db.select().from(memberAuthTable).where(eq(memberAuthTable.email, email.toLowerCase())).limit(1);
...
router.get("/auth/me", async (req, res): Promise<void> => {
  ...
  const [member] = await db.select().from(memberAuthTable).where(eq(memberAuthTable.token, token)).limit(1);
