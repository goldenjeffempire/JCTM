...
import { User, LogIn, LogOut, BookOpen, Heart, ChevronRight, CheckCircle, Eye, EyeOff, Church } from "lucide-react";

const BASE = import.meta.env.BASE_URL.replace(/\/$/, "");

interface Member { id: number; firstName: string; lastName: string; email: string; phone?: string | null; createdAt: string; }
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${BASE}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.json())
        .then(data => { if (data.id) { setMember(data); setView("dashboard"); } })
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${BASE}/api/auth/register`, {
        ...
        body: JSON.stringify({ firstName: regForm.firstName, lastName: regForm.lastName, email: regForm.email, password: regForm.password, phone: regForm.phone || undefined }),
      });
      const data = await res.json();
      if (res.ok) { localStorage.setItem("jctm_token", data.token); setMember(data.member); setView("dashboard"); }
            ...
            {view === "dashboard" && member ? (
              <motion.div key="dashboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
                <div className="glass-panel rounded-2xl p-7 border border-accent/20 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Member since {new Date(member.createdAt).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
                  <div className="mt-4 inline-flex items-center gap-2 text-xs text-accent font-medium">
                    <CheckCircle className="h-3.5 w-3.5" /> Verified Digital Member
                  </div>
                </div>
                ...
                <div className="glass-panel rounded-2xl p-6 border border-border/50">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { icon: <ChevronRight className="h-4 w-4" />, label: "Correction Timeline", desc: "Study the 5 corrections", href: "/timeline" },
                    ].map(item => (
                      <a key={item.label} href={item.href} className="flex items-center gap-4 p-3 rounded-xl hover:bg-muted/50 transition-colors group">
                        <div className="w-9 h-9 rounded-lg bg-accent/10 flex items-center justify-center text-accent group-hover:bg-accent group-hover:text-white transition-colors">{item.icon}</div>
                ...
                <Button onClick={handleLogout} variant="outline" className="w-full rounded-full h-11 border-red-200 text-red-600 hover:bg-red-50 flex items-center gap-2">
              ...
              <motion.div key="register" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="glass-panel rounded-2xl p-8 border border-border/50">
                  <div className="flex gap-2 mb-6 p-1 bg-muted rounded-full">
                    <button onClick={() => { setView("register"); setError(""); }} className="flex-1 py-2 rounded-full text-sm font-medium bg-white shadow text-primary">Join Now</button>
                    <button onClick={() => { setView("login"); setError(""); }} className="flex-1 py-2 rounded-full text-sm font-medium text-muted-foreground hover:text-primary">Sign In</button>
                  </div>
                  <form onSubmit={handleRegister} className="space-y-4">
                    ...
                    <div><label className="text-xs font-medium text-primary mb-1 block">Phone (optional)</label><Input type="tel" value={regForm.phone} onChange={e => setRegForm(p => ({ ...p, phone: e.target.value }))} placeholder="+234..." className="bg-white" /></div>
              ...
              <motion.div key="login" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="glass-panel rounded-2xl p-8 border border-border/50">
                  <div className="flex gap-2 mb-6 p-1 bg-muted rounded-full">
                    <button onClick={() => { setView("register"); setError(""); }} className="flex-1 py-2 rounded-full text-sm font-medium text-muted-foreground hover:text-primary">Join Now</button>
